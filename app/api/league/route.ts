...
async function fetchClassicStandings(leagueId: string) {
  ...
  while (true) {
    ...
    if (hasNext) page += 1;
      ...
      // dorzucamy new_entries (przedsezonowe zapisy)
      const newEntries = data?.new_entries?.results ?? [];
      // mapujemy je do formatu jak standings (rank i totals jeszcze nie istnieją)
      const mapped = newEntries.map((n: any, idx: number) => ({
        entry: n.entry,
        player_name: n.entry_name || n.player_first_name + ' ' + n.player_last_name,
        entry_name: n.entry_name || '—',
        total: 0,
        rank: 999999 + idx, // żeby nie mieszać się z prawdziwym rankingiem
        event_total: 0,
      }));
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // bezpieczne mapowanie + sort: najpierw prawdziwy rank, potem „nowi”
    const entries = (raw || [])
      .map((r: any) => ({
        entry: r.entry,
    ...
    const pre_season = entries.every(e => e.total === 0); // jeśli wszyscy mają 0, to najpewniej przed startem
