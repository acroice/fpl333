"""Rdzen logiki: FPL API -> snapshot tabeli ligi -> BigQuery.

Wydzielone z ingest_league_snapshot.py, zeby ta sama logika mogla byc uruchamiana
na dwa sposoby bez duplikacji kodu:
  - recznie z terminala (ingest_league_snapshot.py, do nauki/debugowania)
  - automatycznie jako Cloud Function wywolywana przez Cloud Scheduler (main.py)

Pelne uzasadnienie decyzji (load job zamiast streaming insert, partycjonowanie po
snapshot_ts, dlaczego maly zakres danych) - patrz komentarz w ingest_league_snapshot.py.
"""
...
def ensure_dataset_and_table(client: bigquery.Client) -> bigquery.TableReference:
    ...
    try:
        ...
        table.description = "Jeden snapshot tabeli ligi = jedno pobranie. Historia, nie stan biezacy."
...
def run_ingest() -> dict:
    """Pelny przebieg: pobierz standings -> zapisz do BigQuery. Zwraca podsumowanie (dict),
    wspolne dla trybu CLI (ladny print) i Cloud Function (JSON response)."""
    ...
    return {
        ...
        "table_total_bytes": table.num_bytes,
    }
